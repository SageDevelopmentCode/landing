...
async function main() {
  ...
  if (!token) {
    ...
    console.error('Usage: SUPABASE_ACCESS_TOKEN=your_token node scripts/generate-types-with-token.js');
    process.exit(1);
  }
  ...
  const outputPath = path.join(__dirname, '..', 'app', 'types', 'database.types.ts');
  const schemas = 'waitlist,contact,admin,parent_app';
