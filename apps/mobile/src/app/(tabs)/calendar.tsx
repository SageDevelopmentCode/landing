...
// ---------------------------------------------------------------------------
// Event detail (inside bottom sheet)
// ---------------------------------------------------------------------------
...
export default function CalendarScreen() {
  ...
  const fetchEvents = useCallback(async (date: Date) => {
    ...
    try {
      ...
      notifyError("parent-calendar-fetch", e);
    } finally {
      setLoading(false);
    }
  }, []);

  // Refetch on screen focus (uses ref so it always has the latest date)
  useFocusEffect(
    useCallback(() => {
  ...
  // When navigated to with a selectDate param, jump to that month and select the date.
  useEffect(() => {
  ...
  // Weekly strip: Mon–Fri of the current week
  const sunday = new Date(currentDate);
  sunday.setDate(currentDate.getDate() - currentDate.getDay());
  const weekDays = Array.from({ length: 5 }, (_, i) => {
    const d = new Date(sunday);
    d.setDate(sunday.getDate() + i + 1); // Mon=1 … Fri=5
    return d;
  });
  ...
  // -------------------------------------------------------------------------
  // Swipe gesture for weekly navigation
  // -------------------------------------------------------------------------
