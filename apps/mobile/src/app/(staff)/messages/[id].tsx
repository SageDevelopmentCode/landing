...
import { Brand, BottomTabInset, FontFamilies, floatingTabBarStyle } from "@/constants/theme";
...
export default function StaffChatScreen() {
  ...
  const { id, otherUserName, otherUserAvatar: otherUserAvatarParam, otherUserId } = useLocalSearchParams<{
    id: string;
  ...
  async function uploadImage(userId: string, conversationId: string, img: PendingImage) {
  ...
  async function uploadFile(userId: string, conversationId: string, file: PendingFile) {
    const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, "_");
    const path = `${userId}/${conversationId}/${Date.now()}-${safeName}`;
    const url = await uploadToStorage("message-files", path, file.uri, file.mimeType);
  ...
  async function handleSend() {
    ...
    try {
      ...
      notifyError('staff-send-message', e);
      Alert.alert("Error", `Failed to send message: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      setSending(false);
    }
  }
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <View style={styles.header}>
        <Pressable
          onPress={() => router.replace("/(staff)/messages")}
      ...
      >
              ...
              >
                <View
                  style={[
                    ...
                    item.image_url && !item.body ? styles.bubbleImageOnly : null,
                    item.file_url && !item.body && !item.image_url ? styles.bubbleFileOnly : null,
                  ]}
                >
                  {item.image_url ? (
                    <Pressable onPress={() => setSelectedImageUrl(item.image_url!)}>
                      <Image
        ...
        <View style={styles.inputWrapper}>
          ...
          >
            ...
            <Pressable
              style={styles.attachMenuItem}
              onPress={() => { setAttachMenuOpen(false); pickFile(); }}
            >
              <Ionicons name="attach-outline" size={22} color={Brand.sage700} />
              <Text style={styles.attachMenuLabel}>Upload File</Text>
            </Pressable>
          </Animated.View>
          <View style={[styles.inputRow, (pendingImage || pendingFile) && styles.inputRowNoTopBorder]}>
            <Pressable
            ...
            <Pressable
              style={({ pressed }) => [
                styles.sendButton,
                (!inputText.trim() && !pendingImage && !pendingFile) && styles.sendButtonDisabled,
                sending && styles.sendButtonDisabled,
                pressed && styles.pressed,
              ]}
              onPress={handleSend}
              disabled={(!inputText.trim() && !pendingImage && !pendingFile) || sending || !currentUserId}
            >
              {sending ? (
