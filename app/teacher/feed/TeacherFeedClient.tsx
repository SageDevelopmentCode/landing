    ...
    <div className="flex flex-wrap gap-1.5">
      {reactions.map((r) => (
        <motion.button
          ...
          onClick={(e) => {
            e.stopPropagation();
            if (currentUserId) onToggle(r.emoji);
          }}
          className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium transition-colors border cursor-pointer ${
            r.reacted_by_me
...
export default function TeacherFeedClient({
  ...
  function handleCommentAdded(postId: string, comment: FeedCommentRow) {
    setPosts((prev) =>
      prev.map((p) => {
        if (p.id !== postId) return p;
