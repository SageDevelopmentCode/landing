    ...
    <>
      ...
      >
        ...
        >
          <div>
            {timeLabel && (
              <p className="text-xs mb-0.5" style={{ color: colors.textSecondary }}>{timeLabel}</p>
            )}
            <h2 className={`text-base font-bold ${merriweather.className}`} style={{ color: colors.textPrimary }}>
          ...
          <motion.button
            ...
            style={{
              ...
              color: colors.textSecondary, backgroundColor: "white", display: "flex",
            }}
          >
            <X className="w-4 h-4" />
          </motion.button>
              ...
              >
                ...
                {ev.category && (
                  <span
                    ...
                    style={{ backgroundColor: `${ev.color}18`, color: ev.color, borderRadius: radius.full }}
                  >
                    {ev.category}
                  </span>
                )}
                  ...
                  <div className="flex items-center gap-1.5 mt-2">
                    <span
                      ...
                      style={{
                        ...
                        backgroundColor: `${ev.color}30`, color: ev.color,
                        border: `1.5px solid ${ev.color}50`, flexShrink: 0,
                      }}
                    >
        ...
        >
          <div className="flex items-start justify-between mb-3">
            ...
            <motion.button
              ...
              style={{
                padding: 6, borderRadius: 8, border: "none", cursor: "pointer",
                backgroundColor: "rgba(255,255,255,0.20)", color: "white", display: "flex", flexShrink: 0,
              }}
            >
              <X className="w-4 h-4" />
            </motion.button>
          </div>
          <h2 className={`text-xl font-bold leading-snug ${merriweather.className}`} style={{ color: "white" }}>
            {event.title}
          </h2>
            ...
            <div className="flex flex-wrap gap-1.5 mt-3">
              {(event.programs ?? []).map((p) => (
                <span key={p} className="px-2.5 py-1 text-[10px] font-medium"
                  style={{ backgroundColor: "rgba(255,255,255,0.18)", color: "rgba(255,255,255,0.90)", borderRadius: radius.full }}>
                  {p}
                </span>
              ))}
            </div>
            ...
            {event.recurrence && event.recurrence !== "None" && (
              <motion.div variants={fadeUpVariants} className="flex items-start gap-3" style={{ padding: "12px 0", borderBottom: `1px solid ${colors.border}` }}>
                ...
                <div className="min-w-0 pt-1">
                  ...
                  <p className="text-sm" style={{ color: colors.textPrimary }}>
                    Repeats {event.recurrence.toLowerCase()}{event.recurrence_end_date ? ` until ${event.recurrence_end_date}` : ""}
                  </p>
                </div>
              </motion.div>
            )}
                  ...
                  <div className="flex flex-col gap-1">
                    {(event.attachment_links ?? []).map((link, i) => (
                      <a key={i} href={link} target="_blank" rel="noopener noreferrer"
                        className="text-xs truncate hover:underline" style={{ color: colors.mistyForest }}>{link}</a>
                    ))}
                  </div>
                </div>
              </motion.div>
      ...
      <div className="flex h-full" style={{ minHeight: 0 }}>
        ...
        <aside className="flex-shrink-0 flex flex-col" style={{ width: 220, borderRight: `1px solid ${colors.border}`, backgroundColor: "white" }}>
          ...
          <div className="flex items-center gap-2 px-4 flex-shrink-0"
            ...
            <span className={`text-[13px] font-bold ${merriweather.className}`} style={{ color: colors.mistyForest }}>My Calendar</span>
          </div>
              ...
              return (
                <motion.button key={key} onClick={() => handleProgramSelect(key)}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {active && <motion.div layoutId="teacher-program-indicator" style={{ marginLeft: "auto", width: 7, height: 7, borderRadius: "50%", backgroundColor: colors.mistyForest, flexShrink: 0 }} />}
                  </div>
          ...
          <div className="flex-1 flex flex-col"
                  ...
                  {(["monthly", "weekly"] as ViewMode[]).map((v) => {
                    ...
                    return (
                      <button key={v} onClick={() => setView(v)}
                        ...
                        {active && (
                          <motion.div layoutId="teacher-view-toggle"
            ...
            <AnimatePresence mode="wait">
              <motion.div key={view + navLabel} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.18, ease: "easeOut" as const }} className="flex-1 overflow-hidden flex flex-col">
                {view === "monthly" ? (
                  <MonthlyGrid days={monthDays} currentMonth={currentDate.getMonth()} today={today} onAddEvent={openAddEvent} onViewEvent={setViewEvent} events={events} />
                ) : (
                  <WeeklyGrid weekDays={weekDays} today={today} onAddEvent={openAddEvent} onViewEvent={setViewEvent} onViewOverlap={setOverlapEvents} events={events} usersMap={usersMap} />
                )}
              </motion.div>
            </AnimatePresence>
          </div>
