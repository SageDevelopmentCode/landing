...
} from "@/app/actions/teacherNotes";
import {
  getStudentAttendance,
  type CheckInRecord,
} from "@/app/actions/getStudentAttendance";
...
import {
  getParentUsersByEmails,
  type ParentUserInfo,
} from "@/app/actions/getParentUsersByEmails";
import {
  getConversations,
...
const TABS = [
  ...
  { id: "parent-communication", label: "Parent Messages", icon: MessageCircle },
...
function SkeletonBlock({ className }: { className?: string }) {
  return (
    <div
      className={`bg-gray-100 rounded animate-pulse ${className ?? "h-4 w-full"}`}
    />
  );
}
    ...
    <div className="flex items-center gap-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        {href ? (
          <a
            href={href}
            className="text-sm text-[#3d6b4f] font-medium truncate hover:underline"
          >
            {value}
          </a>
        ) : (
      ...
      {name && (
        <div className="flex items-center gap-3 pb-3 mb-1 border-b border-gray-100">
          ...
          <div>
            ...
            {relationship && (
              <p className="text-xs text-gray-400 capitalize">{relationship}</p>
            )}
          </div>
        </div>
      )}
        ...
        rows={[
          {
            ...
            href: guardian1?.email ? `mailto:${guardian1.email}` : undefined,
          },
          {
            icon: phoneIcon,
            label: "Cell Phone",
            value: guardian1?.cellPhone ?? "",
            href: guardian1?.cellPhone
              ? `tel:${guardian1.cellPhone}`
              : undefined,
          },
          {
            icon: workPhoneIcon,
            label: "Work Phone",
            value: guardian1?.workPhone ?? "",
            href: guardian1?.workPhone
              ? `tel:${guardian1.workPhone}`
              : undefined,
          },
        ]}
      />
        ...
        rows={[
          {
            ...
            href: guardian2?.email ? `mailto:${guardian2.email}` : undefined,
          },
          {
            icon: phoneIcon,
            label: "Cell Phone",
            value: guardian2?.cellPhone ?? "",
            href: guardian2?.cellPhone
              ? `tel:${guardian2.cellPhone}`
              : undefined,
          },
          {
            icon: workPhoneIcon,
            label: "Work Phone",
            value: guardian2?.workPhone ?? "",
            href: guardian2?.workPhone
              ? `tel:${guardian2.workPhone}`
              : undefined,
          },
        ]}
      />
        ...
        rows={[
          {
            ...
            href: secondary.phone ? `tel:${secondary.phone}` : undefined,
          },
        ]}
      />
    </>
...
function TeacherNotesTab({ studentId }: { studentId: string }) {
  ...
  const [selectedNote, setSelectedNote] = useState<TeacherNoteRecord | null>(
    null,
  );
        ...
        {loadingNotes ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="border border-gray-100 rounded-2xl px-5 py-4 space-y-2 animate-pulse"
              >
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">
              Add your first observation for this student
            </p>
          </div>
        ) : (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`text-xs font-medium px-2 py-0.5 rounded-full ${CATEGORY_BADGE[note.category]}`}
                      >
                        {
                          NOTE_CATEGORIES.find((c) => c.id === note.category)
                            ?.label
                        }
                      </span>
                      {note.is_shared ? (
                  ...
                  {note.teacher_note_attachments.length > 0 && (
                    <div className="mt-2 flex items-center gap-1.5">
                      ...
                      <span className="text-xs text-gray-400">
                        {note.teacher_note_attachments.length} attachment
                        {note.teacher_note_attachments.length > 1 ? "s" : ""}
                      </span>
                    </div>
                  )}
                </div>
                ...
                {isConfirmDelete ? (
                  ...
                  <div className="space-y-6">
                    ...
                    {selectedNote.teacher_note_attachments.length > 0 && (
                      <div>
                        ...
                        <div className="space-y-2">
                          {selectedNote.teacher_note_attachments.some(
                            (a) => a.file_type === "image",
                          ) && (
                            <div className="grid grid-cols-3 gap-2">
                              {selectedNote.teacher_note_attachments
                                ...
                                .map((att) => (
                                  <img
                                    ...
                                    src={att.url}
                                    alt={att.file_name}
                                    className="h-24 w-full object-cover rounded-lg border border-gray-100"
                                  />
                                ))}
                            </div>
                          )}
                          {selectedNote.teacher_note_attachments
                            .filter((a) => a.file_type === "file")
                            .map((att) => (
                              <div
                                key={att.id}
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span
                        className={`text-xs font-medium px-2 py-0.5 rounded-full ${CATEGORY_BADGE[selectedNote.category]}`}
                      >
                        {
                                ...
                                .map((att) => (
                                  ...
                                  >
                                    <img
                                      src={att.url}
                                      alt={att.file_name}
                                      className="h-32 w-full object-cover rounded-xl border border-gray-100 hover:opacity-90 transition-opacity"
                                    />
                                  </a>
                                ))}
                            </div>
            ...
            {sidebarLoading ? (
              ...
              <>
                <SidebarSection title="Checked In By">
                  ...
                  <SidebarField
                    label="Cell Phone"
                    value={sidebarUser.g1_cell_phone}
                  />
                </SidebarSection>
                {sidebarUser.g2_full_name && (
                  <SidebarSection title="Guardian 2">
                    <SidebarField
                      label="Name"
                      value={sidebarUser.g2_full_name}
                    />
                    <SidebarField
                      label="Relationship"
                      value={sidebarUser.g2_relationship}
                    />
                    <SidebarField label="Email" value={sidebarUser.g2_email} />
                    <SidebarField
                      label="Cell Phone"
                      value={sidebarUser.g2_cell_phone}
                    />
                  </SidebarSection>
                )}
              </>
    ...
    <PlaceholderCard title="Portfolio">
      <div className="grid grid-cols-2 gap-3">
        {projects.map((p, i) => (
          <div
            key={i}
            className="border border-gray-100 rounded-xl px-3 py-3 bg-gray-50"
          >
            <p className="text-sm font-medium text-gray-800 mb-1.5">
              {p.title}
            </p>
            <span
              className={`text-xs font-medium px-2 py-0.5 rounded-full ${subjectColor[p.subject] ?? "bg-gray-100 text-gray-500"}`}
            >
              {p.subject}
            </span>
          </div>
  ...
  useEffect(() => {
    ...
    if (detail.g1_email)
      rawParents.push({
        ...
        nameFromApp: detail.g1_full_name ?? detail.g1_email,
      });
    if (detail.g2_email)
      rawParents.push({
        email: detail.g2_email,
        nameFromApp: detail.g2_full_name ?? detail.g2_email,
      });
    ...
    return (
      <div className="flex items-center justify-center py-16">
        <p className="text-sm text-gray-400 font-body">
          No guardian contact info available.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      {parents.map((p) => {
        ...
        const avatarColor = p.userInfo
          ? colorForId(p.userInfo.id)
          : "bg-gray-200";
        ...
        return (
          <button
            ...
            disabled={!canMessage}
            onClick={() =>
              canMessage && openMessaging(p.userInfo!.id, displayName)
            }
          ...
          >
            <div
              className={`${avatarColor} w-10 h-10 rounded-full flex items-center justify-center text-white text-xs font-semibold font-body shrink-0`}
            >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <p className="text-xs text-gray-500 font-body truncate flex-1">
                  {p.conversation?.lastMessage
                    ? (p.conversation.lastMessage.sender_id === teacherId
                        ? "You: "
                        : "") + p.conversation.lastMessage.body
                    : canMessage
                      ? "No messages yet"
                      : `No account found for ${p.email}`}
                </p>
...
export default function StudentsPageClient({
  ...
  const [detail, setDetail] =
    useState<Awaited<ReturnType<typeof getTeacherStudentDetail>>>(null);
  ...
  return (
    <div className="flex flex-1 h-full overflow-hidden">
                ...
                return (
                  ...
                  >
                    {s.profile_image_url ? (
                      ...
                      <div
                        className={`${colorForId(s.student_id)} w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-semibold font-body shrink-0`}
                      >
                        {initialsFor(s.name ?? "?")}
                      </div>
                    )}
      ...
      <div className="flex flex-1 h-full overflow-hidden bg-white">
        {/* Content area */}
        <div
          className={`flex-1 overflow-y-auto ${activeTab === "parent-communication" ? "" : "px-8 py-8"}`}
        >
          {!selectedStudent ? (
              ...
              {activeTab !== "parent-communication" && (
                <div className="mb-6">
                  ...
                  <p className="text-sm text-gray-400 mt-0.5">
                    {PROGRAM_LABELS[selectedStudent.program] ??
                ...
                <>
                  <SidebarSection title="Student Info">
                    ...
                    <SidebarField
                      ...
                      value={detail?.special_interests}
                    />
                  </SidebarSection>
                  ...
                  <SidebarSection title="Learning Profile">
                    <SidebarField
                      ...
                      value={detail?.learning_style}
                    />
                    <SidebarField
                      label="Strengths & Interests"
                      value={detail?.strengths_interests}
                    />
                    <SidebarField
                      label="Current Challenges"
                      value={detail?.current_challenges}
                    />
                    <SidebarField
                      label="Dysregulation Response"
                      value={detail?.dysregulation_response}
                    />
                    <SidebarField
                      label="Regulation Strategies"
                      value={detail?.regulation_strategies}
                    />
                    <SidebarField
                      label="Activities to Avoid"
                      value={detail?.activities_to_avoid}
                    />
                  </SidebarSection>
                  ...
                  <SidebarSection title="Health Notes">
                    <SidebarField
                      ...
                      value={detail?.has_medical_conditions}
                    />
                    <SidebarField
                      label="Medical Description"
                      value={detail?.medical_conditions_description}
                    />
                    <SidebarField
                      label="Has Allergies"
                      value={detail?.has_allergies}
                    />
                    <SidebarField
                      label="Allergies Description"
                      value={detail?.allergies_description}
                    />
                    <SidebarField
                      label="Emergency Medications"
                      value={detail?.has_emergency_medications}
                    />
                    <SidebarField
                      label="Emergency Medications Description"
                      value={detail?.emergency_medications_description}
                    />
                    <SidebarField
                      label="Needs Aide"
                      value={detail?.needs_aide}
                    />
                    <SidebarField
                      label="Aide Description"
                      value={detail?.needs_aide_description}
                    />
                  </SidebarSection>
                  ...
                  <SidebarSection title="History">
                    <SidebarField
                      label="History Flags"
                      value={detail?.history_flags}
                    />
                    <SidebarField
                      label="History Explanation"
                      value={detail?.history_explanation}
                    />
                  </SidebarSection>
                </>
              )}
