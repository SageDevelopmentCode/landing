...
import { signUpParentQuickApply } from "@/app/actions/signUpParentQuickApply";
import { loginParentQuickApply } from "@/app/actions/loginParentQuickApply";
...
function CreateMode({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await signUpParentQuickApply(fullName, email, password);
...
function LoginMode({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await loginParentQuickApply(email, password);
    if (result?.error) {
      setError(result.error);
      setLoading(false);
    } else if (result?.redirectTo) {
