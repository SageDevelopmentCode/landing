...
import {
  ...
  getTeacherSessionsByUserId,
  clockInByUserId,
  clockOutByUserId,
  deleteSessionByUserId,
  updateSessionNoteByUserId,
} from "@/app/actions/timeclock";
import type { TimeclockTeacher, ClockSession, SessionsByDay } from "@/app/actions/timeclock";
import {
  toISOKey,
  getMondayOfWeek,
  getWeekDays,
...
const LOCKOUT_DELAYS = [0, 1000, 5000, 30000, 60000]; // per fail count
...
function DayView({
  ...
  const dayFull = selectedDate.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" });
              ...
              {isActiveDay ? (
                ...
                <>
                  ...
                  <p className="text-sm text-gray-400 font-body">
                    {completedSessions.length > 0 ? "Clock in again to log more time" : "Tap Clock In to start tracking"}
                  </p>
                </>
              )}
            </div>
...
function MonthView({
  ...
  const loggedCount = cells.filter((d) => d && getDayTotalHours(sessionsByDay[toISOKey(d)] ?? []) > 0).length;
  const totalLoggedHours = cells.reduce((acc, d) => d ? acc + getDayTotalHours(sessionsByDay[toISOKey(d)] ?? []) : acc, 0);
    ...
    >
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-1">
          <button onClick={prevMonth} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors cursor-pointer"><ChevronLeft className="w-4 h-4" /></button>
          <button onClick={nextMonth} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors cursor-pointer"><ChevronRight className="w-4 h-4" /></button>
        </div>
      </div>
        ...
        {cells.map((d, i) => {
          ...
          return (
            <button
              ...
              disabled={isWeekend}
              className={`relative flex flex-col items-center justify-start rounded-xl py-2 px-1 transition-all cursor-pointer
            ...
            >
              <span className={`text-sm font-medium font-body leading-none mb-1.5
                ${isToday ? "w-6 h-6 flex items-center justify-center rounded-full bg-[#4a7c59] text-white text-xs" : isLogged ? "text-gray-800" : isFuture ? "text-gray-300" : "text-gray-500"}`}>
                {d.getDate()}
              </span>
              {isLogged && (
...
function SummaryPanel({ sessionsByDay, activeSession }: { sessionsByDay: SessionsByDay; activeSession: ClockSession | null }) {
  const today = new Date();
  const todayKey = toISOKey(today);
  ...
  const monthTotal = monthCells.reduce((acc, d) => d ? acc + getDayTotalHours(sessionsByDay[toISOKey(d)] ?? []) : acc, 0);
...
function PinEntry({ onSuccess }: { onSuccess: (teacher: TimeclockTeacher) => void }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f5f5f0] px-4">
      <motion.div
        ...
        className="bg-white rounded-3xl shadow-lg border border-gray-100 p-10 w-full max-w-sm flex flex-col items-center gap-8"
      >
        {/* Logo */}
        <div className="flex flex-col items-center gap-3">
          <Image src="/assets/Logo.png" alt="Sage Field" width={56} height={56} className="object-contain" />
          <div className="text-center">
            <p className="text-base font-semibold text-gray-800 font-heading">Sage Field</p>
            <p className="text-xs text-gray-400 font-body mt-0.5">Employee Time Clock</p>
          </div>
        </div>

        {/* PIN dots */}
        <div className="flex flex-col items-center gap-4">
          <p className="text-sm font-medium text-gray-500 font-body">Enter your employee code</p>
          <motion.div
            ...
            {Array.from({ length: 5 }).map((_, i) => (
              <div
                ...
                className={`w-3.5 h-3.5 rounded-full transition-all duration-150 ${
                  i < pin.length ? "bg-[#4a7c59] scale-110" : "bg-gray-200"
                }`}
              />
            ))}
          </motion.div>
          <AnimatePresence>
            {error && (
              <motion.p
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0 }}
                className="text-xs text-red-500 font-body text-center"
              >
                {isLocked && lockCountdown > 0 ? `Too many attempts. Try again in ${lockCountdown}s.` : error}
              </motion.p>
            )}
          </AnimatePresence>
        </div>
          ...
          {keys.map((key) => {
            if (key === "backspace") {
              return (
                <button
                  ...
                  className="h-14 rounded-2xl flex items-center justify-center bg-gray-100 hover:bg-gray-200 text-gray-500 transition-colors disabled:opacity-40 cursor-pointer"
                >
            ...
            if (key === "enter") {
              return (
                <button
                  key="enter"
                  onClick={() => void submit()}
                  disabled={pin.length !== 5 || isLocked || loading}
                  className="h-14 rounded-2xl flex items-center justify-center bg-[#4a7c59] hover:bg-[#3d6b4a] text-white text-sm font-semibold font-body transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                >
                  {loading ? (
            ...
            return (
              <button
                ...
                disabled={isLocked || loading || pin.length >= 5}
                className="h-14 rounded-2xl flex items-center justify-center bg-gray-50 hover:bg-gray-100 border border-gray-100 text-gray-800 text-xl font-semibold font-heading transition-colors disabled:opacity-40 cursor-pointer"
              >
                {key}
              </button>
            );
...
function ClockView({
  ...
  const resetInactivity = useCallback(() => {
    ...
    inactivityTimer.current = setTimeout(() => onSwitch(), INACTIVITY_MS);
  }, [onSwitch]);
  ...
  useEffect(() => {
    getTeacherSessionsByUserId(teacher.id).then((sessions) => {
  ...
  async function handleClockIn() {
    resetInactivity();
    const session = await clockInByUserId(teacher.id, teacher.full_name);
    if (!session) return;
    const dateKey = toISOKey(new Date(session.clockInAt));
    setSessionsByDay((prev) => ({ ...prev, [dateKey]: [...(prev[dateKey] ?? []), session] }));
  }

  async function handleClockOut() {
    resetInactivity();
    if (!activeSession) return;
    const clockOutAt = await clockOutByUserId(activeSession.id, teacher.id, teacher.full_name);
    if (!clockOutAt) return;
    const dateKey = toISOKey(new Date(activeSession.clockInAt));
    setSessionsByDay((prev) => ({
      ...prev,
      [dateKey]: (prev[dateKey] ?? []).map((s) => s.id === activeSession.id ? { ...s, clockOutAt } : s),
    }));
  }

  function handleDeleteSession(id: string) {
    resetInactivity();
    deleteSessionByUserId(id, teacher.id);
    setSessionsByDay((prev) => {
      const next = { ...prev };
  ...
  function handleNoteChange(id: string, note: string) {
    resetInactivity();
    updateSessionNoteByUserId(id, note, teacher.id);
    setSessionsByDay((prev) => {
      const next = { ...prev };
      for (const key of Object.keys(next)) {
    ...
    <div className="flex flex-col h-screen bg-[#f5f5f0]">
      ...
      <header className="bg-white border-b border-gray-100 px-6 py-3 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <Image src="/assets/Logo.png" alt="Sage Field" width={36} height={36} className="object-contain" />
          <div className="flex items-center gap-3">
            {teacher.profile_image_url ? (
              <Image
                ...
                width={32}
                height={32}
                className="rounded-full object-cover w-8 h-8 border border-gray-100"
              />
            ) : (
              <div className="w-8 h-8 rounded-full bg-[#4a7c59]/10 flex items-center justify-center">
                <UserRound className="w-4 h-4 text-[#4a7c59]" />
              </div>
            )}
            <div>
              <p className="text-sm font-semibold text-gray-800 font-body leading-none">{teacher.full_name ?? "Employee"}</p>
              {activeSession && (
                <p className="text-[11px] text-[#4a7c59] font-body mt-0.5 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#4a7c59] animate-pulse inline-block" />
                  Clocked in
                </p>
              )}
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-gray-500 hover:text-gray-800 hover:bg-gray-100 transition-colors cursor-pointer font-body"
        >
      ...
      {loadingSessions ? (
        <div className="flex-1 flex items-center justify-center">
          <span className="w-6 h-6 border-2 border-[#4a7c59]/30 border-t-[#4a7c59] rounded-full animate-spin" />
        </div>
      ) : (
              ...
              <div className="flex items-center gap-3 shrink-0">
                ...
                <div className="flex items-center gap-1 bg-gray-100 rounded-xl p-1">
                  {views.map(({ id, label, icon }) => (
                    <button
                      key={id}
                      onClick={() => { setView(id); resetInactivity(); }}
...
export default function TimeclockClient() {
  const [teacher, setTeacher] = useState<TimeclockTeacher | null>(null);

  if (!teacher) {
    return <PinEntry onSuccess={(t) => setTeacher(t)} />;
  }

  return (
    <ClockView
      teacher={teacher}
      onSwitch={() => setTeacher(null)}
    />
  );
}
