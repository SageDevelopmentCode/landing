...
export default async function AftercareSuccessPage({ searchParams }: Props) {
  const { session_id } = await searchParams;

  let amountFormatted = "";
  let planLabel = "After Care";
  ...
  if (session_id) {
    try {
      ...
      if (planType === "monthly") {
        ...
        planLabel =
          months.length > 0
            ? `After Care for ${months.join(", ")}`
            : "After Care (Monthly)";
      } else {
        const days = session.metadata?.selected_days
          ? session.metadata.selected_days.split(",").filter(Boolean)
          : [];
        planLabel = `After Care — ${days.length} day${days.length !== 1 ? "s" : ""}`;
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-welcome-bg flex flex-col items-center justify-center px-6 py-24">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
        ...
        <p className="text-sm text-gray-500 font-body mb-2">
          Thank you! Your child is enrolled in {planLabel}. We&apos;ll be in touch with details soon.
        </p>
