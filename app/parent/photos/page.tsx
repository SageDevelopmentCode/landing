...
import ParentPhotosClient from "./ParentPhotosClient";
import { getAllSchoolPhotos } from "@/app/actions/photos";
...
export default async function ParentPhotosPage() {
  ...
  const [{ data: adminUser }, initialPhotos, { data: enrolledCheck }] = await Promise.all([
    adminClient
      .schema("admin")
      .from("users")
  ...
  return (
    <div className="bg-welcome-bg min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex overflow-hidden">
        <ParentPhotosClient initialPhotos={initialPhotos} />
      </main>
    </div>
  );
}
