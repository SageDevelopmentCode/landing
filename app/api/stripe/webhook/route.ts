...
export async function POST(request: NextRequest) {
    ...
    if (session.metadata?.payment_type === "registration_fee") {
      ...
      try {
        await notifySchoolYearTuitionPayment({
          ...
          parentEmailAddr:
            session.metadata?.parent_email ?? session.customer_email ?? "N/A",
          amountCents: session.amount_total ?? 0,
          selectedMonthsRaw: session.metadata?.selected_months ?? "",
        });
      } catch (err) {
  ...
  // Mobile Payment Sheet (metadata.mobile=true): match web Checkout behavior —
  // record billing + notify on processing (ACH submitted), not only on succeeded.
  if (
