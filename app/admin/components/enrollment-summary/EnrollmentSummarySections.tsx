...
} from "@/app/actions/getAdminEnrollmentData";
import type {
  SignatureMap,
  EnrollmentSignature,
...
import { ENROLLMENT_ITEM_TITLES } from "@/app/lib/enrollment-checklist";
...
export type EnrollmentSummaryApplication = {
  id: string;
  user_id: string;
...
export type EnrollmentItemRenderProps = {
  itemId: number;
  app: EnrollmentSummaryApplication;
  signatureMap: SignatureMap;
  healthInfo: StudentHealthInfo | null;
  medicationPlan: StudentMedicationPlan | null;
  medications: StudentMedication[];
  photoConsentLevel: string | null;
  pickupPlan: StudentAuthorizedPickupPlan | null;
  pickupPersons: StudentAuthorizedPickupPerson[];
  registrationFeePaid: boolean;
  healthStatementOptionType: string | null;
  religiousExemptionCount: number;
  immunizationFileNames: string[];
};

export function SignatureFontProvider({
  children,
  className = "",
}: {
  ...
  return (
    <div className={`${dancingScript.variable} ${className}`}>{children}</div>
  );
}

export { ENROLLMENT_ITEM_TITLES };
...
export function SectionHeader({ title, number }: { title: string; number?: string }) {
  return (
...
export function ReadOnlySignatureBlock({
  sig,
}: {
  ...
  if (!sig) {
    return (
      <div className="summary-signature-block mt-2 border border-dashed border-gray-200 rounded-lg px-2 py-1.5 bg-gray-50">
        <p className="text-xs text-gray-400 font-body italic">Not signed</p>
      </div>
    );
  }
  return (
    <div
      className={`summary-signature-block ${dancingScript.variable} mt-2 border border-gray-300 rounded-lg px-2 py-1.5 flex items-center justify-between gap-3`}
    >
      <div className="flex items-center gap-2 min-w-0">
        <CheckCircle className="w-3.5 h-3.5 text-emerald-600 shrink-0 print:hidden" />
        <p
          className="text-lg text-gray-700 truncate print:text-sm"
      ...
      <div className="text-right shrink-0">
        <p className="text-[10px] text-gray-500 font-body">{sig.printed_name}</p>
...
export function HealthFormContent({
  app,
  healthInfo,
  signatureMap,
}: {
...
export function MedicationPlanContent({
  app,
  plan,
  medications,
  signatureMap,
...
export function PhotoReleaseContent({
  consentLevel,
  signatureMap,
}: {
...
export function AssumptionOfRiskContent({
  app,
  signatureMap,
}: {
  app: EnrollmentSummaryApplication;
...
export function AuthorizedPickupContent({
  app,
  plan,
  persons,
  signatureMap,
...
export function HealthStatementContent({
  app,
  optionType,
...
export function ImmunizationSummaryContent({
  fileNames,
}: {
  fileNames: string[];
}) {
  ...
  return (
    <div className="flex flex-col gap-2">
      {fileNames.map((name) => (
        <div
          key={name}
          className="flex items-center gap-3 w-full px-4 py-3 rounded-xl border border-gray-200 bg-white"
        >
...
export function RegistrationFeeSummaryContent({ paid }: { paid: boolean }) {
  return (
    <div className="flex items-center gap-2">
      {paid ? (
...
export function ImmunizationFilesContent({
  parentId,
  studentId,
}: {
  parentId: string;
...
export function renderEnrollmentItemContent(props: EnrollmentItemRenderProps) {
  const {
    itemId,
    app,
    signatureMap,
  ...
  switch (itemId) {
    case 1:
      return (
        <ContractContent contractId={CONTRACT_1_ID} signatureMap={signatureMap} />
      );
    case 2:
      return (
        <ContractContent contractId={CONTRACT_2_ID} signatureMap={signatureMap} />
      );
    case 3:
      return (
        <HealthFormContent
    ...
    case 5:
      return <ImmunizationSummaryContent fileNames={immunizationFileNames} />;
    case 6:
      return (
        <PhotoReleaseContent
          consentLevel={photoConsentLevel}
          signatureMap={signatureMap}
        />
      );
    case 7:
    ...
    case 9:
      return <RegistrationFeeSummaryContent paid={registrationFeePaid} />;
    case 10:
      return (
        <HealthStatementContent
          app={app}
          optionType={healthStatementOptionType}
