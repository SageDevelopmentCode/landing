...
export function ApplicationDetailSidebar({
  ...
  const handleSendHomeschoolDropInTuitionReminder = async () => {
    if (homeschoolDropInTuitionReminderSending || !application.g1_email) return
    setHomeschoolDropInTuitionReminderSending(true)
    setHomeschoolDropInTuitionReminderError(null)
    const result = await sendHomeschoolDropInTuitionReminderEmail({
      g1FullName: application.g1_full_name ?? '',
      childLegalName: application.child_legal_name ?? '',
      email: application.g1_email,
    })
    setHomeschoolDropInTuitionReminderSending(false)
    if (result.success) {
      setHomeschoolDropInTuitionReminderSent(true)
      setEmailThreadKey(k => k + 1)
    } else {
  ...
  const handleSendWeekTwelveNewsletter = async () => {
    if (weekTwelveNewsletterSending || !application.g1_email) return
    setWeekTwelveNewsletterSending(true)
    setWeekTwelveNewsletterError(null)
    const result = await sendSummerWeekTwelveNewsletterEmail({
      g1FullName: application.g1_full_name ?? '',
      childLegalName: application.child_legal_name ?? '',
      email: application.g1_email,
    })
    setWeekTwelveNewsletterSending(false)
    if (result.success) {
      setWeekTwelveNewsletterSent(true)
      setEmailThreadKey(k => k + 1)
  ...
  const handleSendSchoolYearWeekFiveNewsletter = async () => {
    if (schoolYearWeekFiveNewsletterSending || !application.g1_email) return
    setSchoolYearWeekFiveNewsletterSending(true)
    setSchoolYearWeekFiveNewsletterError(null)
    const result = await sendSchoolYearWeekFiveNewsletterEmail({
      g1FullName: application.g1_full_name ?? '',
      childLegalName: application.child_legal_name ?? '',
      email: application.g1_email,
    })
    setSchoolYearWeekFiveNewsletterSending(false)
    if (result.success) {
      setSchoolYearWeekFiveNewsletterSent(true)
      setEmailThreadKey(k => k + 1)
      setTimeout(() => setSchoolYearWeekFiveNewsletterSent(false), 3000)
    } else {
      setSchoolYearWeekFiveNewsletterError(result.error ?? 'Failed to send')
    }
  }
          ...
          <SidebarSection title="Outreach">
            ...
            <div className="flex flex-col gap-3">
              ...
              {outreachTab === 'newsletters' && <>
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={handleSendWeekTwelveNewsletter}
                    disabled={weekTwelveNewsletterSending || weekTwelveNewsletterSent}
                    className="px-3 py-1.5 text-sm font-semibold text-white rounded-lg transition-colors hover:bg-[#234d25] disabled:opacity-50 disabled:cursor-not-allowed"
                    style={{ backgroundColor: '#2C5F2E', border: 'none', borderRadius: '8px' }}
                  >
                    {weekTwelveNewsletterSending ? 'Sending…' : weekTwelveNewsletterSent ? '✓ Sent!' : 'Send Week Twelve Newsletter'}
                  </button>
                  {weekTwelveNewsletterError && <span className="text-xs text-red-600">{weekTwelveNewsletterError}</span>}
                </div>
              </>}
              ...
              {outreachTab === 'schoolYear' && <>
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={handleSendSchoolYearWeekFiveNewsletter}
                    disabled={schoolYearWeekFiveNewsletterSending || schoolYearWeekFiveNewsletterSent}
                    className="px-3 py-1.5 text-sm font-semibold text-white rounded-lg transition-colors hover:bg-[#234d25] disabled:opacity-50 disabled:cursor-not-allowed"
                    style={{ backgroundColor: '#2C5F2E', border: 'none', borderRadius: '8px' }}
                  >
              ...
              {outreachTab === 'other' && <>
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={handleSendHomeschoolDropInTuitionReminder}
                    disabled={homeschoolDropInTuitionReminderSending || homeschoolDropInTuitionReminderSent}
                    className="px-3 py-1.5 text-sm font-semibold text-white rounded-lg transition-colors hover:bg-[#234d25] disabled:opacity-50 disabled:cursor-not-allowed"
                    style={{ backgroundColor: '#2C5F2E', border: 'none', borderRadius: '8px' }}
                  >
