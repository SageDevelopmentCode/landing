...
import type { AdminEnrollmentData, StudentHealthInfo, StudentMedicationPlan, StudentMedication, StudentAuthorizedPickupPlan, StudentAuthorizedPickupPerson } from '../../actions/getAdminEnrollmentData'
...
import { CONTRACT_1_SECTIONS, CONTRACT_2_SECTIONS, CONTRACT_5_SECTIONS } from '@/app/parent/dashboard/contractContent'
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start gap-3">
      <span className="text-xs font-semibold font-body text-gray-500 w-36 flex-shrink-0 pt-0.5">{label}</span>
      <span className="text-sm font-body text-gray-800">{value}</span>
    </div>
  )
}

function ReadOnlySignatureBlock({ sig }: { sig: EnrollmentSignature | undefined }) {
  if (!sig) {
    return (
  ...
  return (
    ...
    >
      ...
      <div className="text-right flex-shrink-0">
        ...
        <p className="text-xs text-gray-400 font-body">{new Date(sig.signed_at).toLocaleDateString()}</p>
      </div>
    </div>
  )
}
        ...
        return (
          <div key={section.id} className="flex flex-col">
            ...
            <div className="flex flex-col gap-2 text-sm text-gray-600 font-body">
              ...
              {section.afterBullets && section.afterBullets.map((p, i) => (
                <p key={i} className="leading-relaxed mt-1">{p}</p>
              ))}
            </div>
...
function HealthFormContent({
  ...
  const dobFormatted = [app.dob_month, app.dob_day, app.dob_year].filter(Boolean).join('/') || null
    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <ReadOnlyField label="Full Name" value={healthInfo?.in_state_contact_name} />
            <ReadOnlyField label="Relationship" value={healthInfo?.in_state_contact_relation} />
            <ReadOnlyField label="Phone" value={healthInfo?.in_state_contact_phone} />
          </div>
        </div>
        <div className="flex flex-col gap-3">
          <p className="text-xs font-semibold text-gray-500 font-body uppercase tracking-wide">Out-of-State Emergency Contact</p>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <ReadOnlyField label="Full Name" value={healthInfo?.out_of_state_contact_name} />
            <ReadOnlyField label="Relationship" value={healthInfo?.out_of_state_contact_relation} />
            <ReadOnlyField label="Phone" value={healthInfo?.out_of_state_contact_phone} />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <ReadOnlyField label="Primary Physician Name" value={healthInfo?.physician_name} />
          <ReadOnlyField label="Clinic/Practice Name" value={healthInfo?.clinic_name} />
          <ReadOnlyField label="Physician Phone" value={healthInfo?.physician_phone} />
          <ReadOnlyField label="Health Insurance Provider" value={healthInfo?.insurance_provider} />
          <ReadOnlyField label="Policy Number" value={healthInfo?.policy_number} />
          <ReadOnlyField label="Group Number" value={healthInfo?.group_number} />
        </div>
        <ReadOnlyField label="Preferred Hospital or Emergency Care Facility" value={healthInfo?.preferred_hospital} />
        <p className="text-sm text-gray-600 font-body leading-relaxed">
          If emergency treatment is needed and I/we cannot be reached, I authorize Sage Field Private School staff to seek necessary medical attention from the nearest licensed physician, hospital, or emergency facility.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        {(app.has_allergies || app.allergies_description) && (
          <div className="bg-rose-50 border border-rose-100 rounded-xl px-4 py-3">
            ...
            <p className="text-sm text-gray-700 font-body">{(app.allergies_description as string) || 'Yes (no description provided)'}</p>
          </div>
        )}
        {(app.has_medical_conditions || app.medical_conditions_description) && (
          <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
            ...
            <p className="text-sm text-gray-700 font-body">{(app.medical_conditions_description as string) || 'Yes (no description provided)'}</p>
          </div>
        )}
        {!app.has_allergies && !app.allergies_description && !app.has_medical_conditions && !app.medical_conditions_description && (
          <div className="bg-gray-50 rounded-xl px-4 py-3">
            <p className="text-sm text-gray-500 font-body">No allergies or medical conditions on file.</p>
          </div>
        )}
        <ReadOnlyField label="Additional health conditions or notes" value={healthInfo?.health_conditions} />
        <div className="flex flex-col gap-2">
          <p className="text-xs font-semibold text-gray-600 font-body">Is your child currently receiving care for an ongoing condition?</p>
          <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold font-body w-fit ${healthInfo?.ongoing_care ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-500'}`}>
            {healthInfo?.ongoing_care ? 'Yes' : 'No'}
          </span>
          {healthInfo?.ongoing_care && healthInfo.ongoing_care_description && (
            <ReadOnlyField label="Description" value={healthInfo.ongoing_care_description} />
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        {(app.has_emergency_medications || app.emergency_medications_description) && (
          <div className="bg-rose-50 border border-rose-100 rounded-xl px-4 py-3">
            <p className="text-xs font-semibold text-rose-700 font-body mb-1">Emergency medications on file</p>
            <p className="text-sm text-gray-700 font-body">{(app.emergency_medications_description as string) || 'Yes (no description provided)'}</p>
          </div>
        )}
        <div className="flex flex-col gap-2">
          <p className="text-xs font-semibold text-gray-600 font-body">Does your child require emergency medication at school?</p>
          <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold font-body w-fit ${healthInfo?.emergency_medication_required ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-500'}`}>
            {healthInfo?.emergency_medication_required ? 'Yes' : 'No'}
          </span>
          {healthInfo?.emergency_medication_required && healthInfo.emergency_medication_description && (
            <ReadOnlyField label="Medication details" value={healthInfo.emergency_medication_description} />
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <p className="text-sm text-gray-600 font-body leading-relaxed">
          Texas law requires all schoolchildren to provide proof of current immunizations or a valid exemption affidavit.
        </p>
        {healthInfo?.immunization_status ? (
          <div className="w-full text-left px-4 py-3 rounded-xl border border-gray-800 bg-gray-800 text-white flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold font-body">
                {healthInfo.immunization_status === 'record' ? 'Immunization record on file' : 'Exemption affidavit'}
              </p>
          ...
          <p className="text-sm text-gray-400 font-body italic">No selection made.</p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <p className="text-sm text-gray-600 font-body leading-relaxed">
          Sage Field Private School staff do not administer prescription or over-the-counter medications. The only exception is for life-saving emergency medications (e.g., EpiPen, inhaler) with prior written authorization. By initialing below, you acknowledge and agree to this policy.
        </p>
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="bg-gray-50 rounded-xl px-4 py-4">
          <p className="text-xs text-gray-500 font-body">
            In case of emergency, school staff will contact the parent/guardian listed above first. If unreachable, staff will contact the emergency contact listed on the student&apos;s health form.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <p className="text-sm text-gray-600 font-body leading-relaxed">
          List all medications your child may require while at school. Include both daily medications and emergency-use medications.
        </p>
        {medications.length === 0 ? (
          <p className="text-sm text-gray-400 font-body italic">No medications listed.</p>
        ) : (
          <div className="flex flex-col gap-6">
            {medications.map((med, index) => (
              <div key={med.id} className="border border-gray-200 rounded-xl px-4 py-4 flex flex-col gap-3 bg-white">
                <p className="text-xs font-semibold text-gray-500 font-body uppercase tracking-wide">Medication {index + 1}</p>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <ReadOnlyField label="Medication Name" value={med.medication_name} />
                  <ReadOnlyField label="Condition / Reason" value={med.condition_reason} />
                  <ReadOnlyField label="Dosage & Frequency" value={med.dosage_frequency} />
                  <ReadOnlyField label="Prescribing Physician" value={med.physician_name} />
                  <ReadOnlyField label="Physician Phone" value={med.physician_phone} />
                  <ReadOnlyField label="Expiration Date" value={med.expiration_date} />
                </div>
                <div className="flex gap-4">
                  <div className="flex items-center gap-2">
                    <span className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 ${med.is_daily ? 'bg-gray-800 border-gray-800' : 'border-gray-300'}`}>
                      {med.is_daily && <span className="w-2 h-2 bg-white rounded-sm" />}
                    </span>
                    <span className="text-xs font-body text-gray-600">Daily medication</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 ${med.is_emergency_only ? 'bg-gray-800 border-gray-800' : 'border-gray-300'}`}>
                      {med.is_emergency_only && <span className="w-2 h-2 bg-white rounded-sm" />}
                    </span>
      ...
      <div className="flex flex-col gap-4">
        ...
        <ReadOnlyField label="Symptoms or situations when medication should be given" value={plan?.emergency_procedure} />
      </div>

      {/* Section 6: Special Instructions */}
      <div className="flex flex-col gap-4">
        <SectionHeader number="6" title="Special Instructions" />
        <ReadOnlyField label="Storage requirements, assistance needed, or other notes" value={plan?.special_instructions} />
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <p className="text-sm text-gray-600 font-body leading-relaxed">
          I hereby authorize Sage Field Private School staff to administer the medication(s) listed above to my child in accordance with the instructions provided. I understand that school staff are not licensed medical professionals and will follow the instructions given to the best of their ability. I release Sage Field Private School and its staff from liability for any adverse reactions that occur when medication is administered as directed.
        </p>
        <p className="text-sm text-gray-600 font-body leading-relaxed">
          I agree to provide medication in its original, clearly labeled container and to update this form whenever medication information changes.
        </p>
...
function PhotoReleaseContent({
  ...
  const CONSENT_OPTIONS: { value: string; label: string; description: string }[] = [
    {
      value: 'FULL',
      label: 'Full Consent',
      description: "My child's image and name may be used in all School materials, including website, social media, newsletters, print, and presentations.",
    },
    {
      value: 'LIMITED',
      label: 'Limited Consent',
      description: "My child's image may be used in internal School materials only (e.g., newsletters sent to enrolled families). My child's image may NOT be used on public-facing platforms such as the website or social media.",
    },
    {
      value: 'NO',
      label: 'No Consent',
      description: 'I do not consent to any photography or video recording of my child for School use. My child will be excluded from group photos and media captures.',
    },
  ]
    ...
    <div className="flex flex-col gap-10">
      ...
      {CONTRACT_5_SECTIONS.filter((s) => s.id !== 5).map((section) => (
        <div key={section.id} className="flex flex-col gap-3">
          <h3 className="text-sm font-bold font-heading text-gray-800 pb-2 border-b border-gray-100">{section.title}</h3>
          {section.paragraphs.map((p, i) => (
            <p key={i} className="text-sm text-gray-600 font-body leading-relaxed">{p}</p>
          ))}
        </div>
      ))}
      ...
      <div className="flex flex-col gap-4">
        <h3 className="text-sm font-bold font-heading text-gray-800 pb-2 border-b border-gray-100">{sec5.title}</h3>
        <p className="text-sm text-gray-600 font-body leading-relaxed">{sec5.paragraphs[0]}</p>
        <div className="flex flex-col gap-3">
            ...
            >
              <span
                className={`flex-shrink-0 w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                  consentLevel === opt.value ? 'border-primary' : 'border-gray-300'
                }`}
              >
                {consentLevel === opt.value && <span className="w-2 h-2 rounded-full bg-primary" />}
              </span>
              <div>
                <p className="text-sm font-semibold font-heading text-gray-800">{opt.label}</p>
                <p className="text-xs text-gray-500 font-body mt-0.5 leading-relaxed">{opt.description}</p>
              </div>
            </div>
          ))}
        </div>
        {!consentLevel && <p className="text-sm text-gray-400 font-body italic">No consent level selected.</p>}
        <ReadOnlySignatureBlock sig={signatureMap[`${CONTRACT_5_ID}-1`]} />
      </div>

      {/* Parent/Guardian Acknowledgment */}
      <div className="flex flex-col gap-4">
        <h3 className="text-sm font-bold font-heading text-gray-800 pb-2 border-b border-gray-100">Parent/Guardian Acknowledgment</h3>
        <p className="text-sm text-gray-600 font-body leading-relaxed">
          By signing below, I confirm that I have read and understood this Photo, Video, and Media Release in its entirety. I acknowledge my selected consent level for my child&apos;s participation in photography and video recording at Sage Field Academy. I understand that this consent may be revoked at any time in writing and that revocation applies to future use only.
        </p>
        <p className="text-sm text-gray-600 font-body leading-relaxed">
          I enter this agreement freely and on behalf of my child, and I have the legal authority to grant or withhold consent as described herein.
        </p>
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-1">
        <p className="text-sm font-semibold font-heading text-gray-800">Sage Field Private School</p>
      ...
      <div className="flex flex-col gap-3">
        ...
        <p className="text-sm text-gray-600 font-body leading-relaxed">
          ...
          <span className="font-semibold text-gray-800">{studentName}</span>&apos;s participation at Sage Field Private School.
        </p>
      </div>
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <ReadOnlyField label="Date of Request" value={plan?.date_of_request} />
          <ReadOnlyField label="Effective Until" value={plan?.effective_until} />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        {persons.length === 0 ? (
          <p className="text-sm text-gray-400 font-body italic">No authorized pickup persons listed.</p>
        ) : (
          <div className="flex flex-col gap-6">
            {persons.map((person, index) => (
              <div key={person.id} className="border border-gray-200 rounded-xl px-4 py-4 flex flex-col gap-3 bg-white">
                <p className="text-xs font-semibold text-gray-500 font-body uppercase tracking-wide">Person {index + 1}</p>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <ReadOnlyField label="Full Name" value={person.full_name} />
                  <ReadOnlyField label="Relationship to Student" value={person.relationship} />
                  <ReadOnlyField label="Phone Number" value={person.phone} />
                  <ReadOnlyField label="Email" value={person.email} />
                  <ReadOnlyField label="DL / State ID Number" value={person.dl_state_id_number} />
                  <ReadOnlyField label="Vehicle Make / Model / Color" value={person.vehicle_info} />
                  <ReadOnlyField label="License Plate / State" value={person.license_plate_state} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <p className="text-sm text-gray-600 font-body leading-relaxed">
          I, the undersigned parent or legal guardian of <span className="font-semibold text-gray-800">{studentName}</span>, hereby authorize the individual(s) listed above to pick up my child from Sage Field LLC. I understand that school staff will verify the identity of any pickup person using a valid government-issued photo ID before releasing my child.
        </p>
        <p className="text-sm text-gray-600 font-body leading-relaxed">
...
function HealthStatementContent({
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* Selected option */}
      <div className="flex flex-col gap-3">
        <p className="text-sm font-semibold font-heading text-gray-800">Selected option:</p>
        <div
        ...
        >
          <span
            className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
              optionType === 'professional' ? 'border-teal-600' : optionType === 'religious' ? 'border-purple-500' : 'border-gray-300'
            }`}
          >
            {optionType && <span className={`w-2 h-2 rounded-full ${optionType === 'professional' ? 'bg-teal-600' : 'bg-purple-500'}`} />}
          </span>
        ...
        {optionType === 'professional' && (
          <div className="flex flex-col gap-3 p-4 rounded-xl border border-teal-100 bg-teal-50/40">
            <p className="text-sm font-semibold font-heading text-gray-800">Option A: Health Care Professional Examination</p>
            <p className="text-sm text-gray-600 font-body leading-relaxed">
            ...
            <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-amber-50 border border-amber-200">
              ...
              <p className="text-xs text-amber-700 font-body">
                After the health care professional signs the form, it should be emailed to{' '}
        ...
        {optionType === 'religious' && (
          <div className="flex flex-col gap-3 p-4 rounded-xl border border-purple-100 bg-purple-50/30">
            <p className="text-sm font-semibold font-heading text-gray-800">Option B: Religious Exemption Affidavit</p>
            <p className="text-sm text-gray-600 font-body leading-relaxed">
              I object to the physical examination of my child on religious grounds. I understand that I must provide a signed, dated, and <span className="font-semibold text-gray-800">notarized</span> affidavit stating my religious objection to such examination.
            </p>
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-semibold font-body px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">
                {religiousExemptionCount} file{religiousExemptionCount !== 1 ? 's' : ''} uploaded
              </span>
            </div>
          </div>
        )}
...
function ImmunizationFilesContent({ parentId, studentId }: { parentId: string; studentId: string }) {
  ...
  async function handleDownload(fileName: string) {
    ...
    const res = await getAdminImmunizationDownloadUrl(`${parentId}/${studentId}/${fileName}`)
    ...
    <div className="flex flex-col gap-2">
      ...
      {files.map((file) => (
        ...
        >
          ...
          <p className="flex-1 text-sm font-body text-gray-700 truncate">{formatFileName(file.name)}</p>
...
export function AdminEnrollmentItemDrawer({
  ...
  const renderContent = () => {
    ...
    switch (itemId!) {
      ...
      case 2:
        return <ContractContent contractId={CONTRACT_2_ID} signatureMap={signatureMap} />
      case 3: {
        const healthInfo = enrollmentData?.healthInfoByStudent[studentId] ?? null
        if (!app) return <p className="text-sm text-gray-400">No application data.</p>
        return <HealthFormContent app={app} healthInfo={healthInfo} signatureMap={signatureMap} />
      }
      case 4: {
        const { plan, medications } = enrollmentData?.medicationPlanByStudent[studentId] ?? { plan: null, medications: [] }
        if (!app) return <p className="text-sm text-gray-400">No application data.</p>
        return <MedicationPlanContent app={app} plan={plan} medications={medications} signatureMap={signatureMap} />
      }
      case 5: {
        if (!app) return <p className="text-sm text-gray-400">No application data.</p>
        return <ImmunizationFilesContent parentId={app.user_id} studentId={studentId} />
      }
      case 6: {
        const consentLevel = enrollmentData?.photoConsentByStudent[studentId] ?? null
        return <PhotoReleaseContent consentLevel={consentLevel} signatureMap={signatureMap} />
      }
      case 7: {
        if (!app) return <p className="text-sm text-gray-400">No application data.</p>
        return <AssumptionOfRiskContent app={app} signatureMap={signatureMap} />
      }
      case 8: {
        const { plan, persons } = enrollmentData?.authorizedPickupByStudent[studentId] ?? { plan: null, persons: [] }
        if (!app) return <p className="text-sm text-gray-400">No application data.</p>
        return <AuthorizedPickupContent app={app} plan={plan} persons={persons} signatureMap={signatureMap} />
      }
      case 9: {
        const paid = enrollmentData?.registrationFeePaidByStudent[studentId] ?? false
        return (
          <div className="flex items-center gap-2">
            {paid ? (
      ...
      case 10: {
        const healthStatement = enrollmentData?.healthStatementByStudent[studentId] ?? null
        const religiousExemptionCount = enrollmentData?.religiousExemptionCountByStudent[studentId] ?? 0
        if (!app) return <p className="text-sm text-gray-400">No application data.</p>
        return (
