...
const TABS = ['Overview', 'Budget', 'Expenses', 'Revenue', 'Taxes', 'Analysis'] as const
...
function OverviewTab({
  ...
  const totalBudget = lineItems.reduce((s, i) => s + Number(i.planned_amount), 0)
  const totalExpenses = expenses.reduce((s, e) => s + Number(e.amount), 0)
  const totalRevenue = income.reduce((s, i) => s + Number(i.amount), 0)
  const netProfit = totalRevenue - totalExpenses
  const profitColor =
    netProfit >= 0 ? colors.successText : colors.errorText
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <MiniStat label="Total Planned Budget" value={fmt(totalBudget)} color={colors.textPrimary} delay={0} />
        <MiniStat label="Total Actual Expenses" value={fmt(totalExpenses)} color={colors.errorText} delay={0.05} />
        <MiniStat label="Total Revenue" value={fmt(totalRevenue)} color={colors.successText} delay={0.1} />
        <MiniStat label="Net Profit / Loss" value={fmt(netProfit)} color={profitColor} delay={0.15} />
      </div>
        ...
        <div className="space-y-4">
          <div style={{ ...cardStyle, padding: '24px' }}>
            ...
            <div className="space-y-2 text-sm">
              {[
                { label: 'Revenue', value: totalRevenue, color: colors.successText },
                { label: 'Expenses', value: -totalExpenses, color: colors.errorText },
              ].map(({ label, value, color }) => (
                <div key={label} className="flex justify-between">
                  <span style={{ color: colors.textSecondary }}>{label}</span>
                  <span style={{ color, fontWeight: 600 }}>{fmt(Math.abs(value))}</span>
                </div>
              ))}
              <div
          ...
          <div style={{ ...cardStyle, padding: '24px' }}>
            ...
            <p className="text-xl font-semibold" style={{ color: colors.warningText }}>
            ...
            <div key={slice.cat} className="space-y-1">
              ...
              >
                <motion.div
                  ...
                  style={{ height: '100%', borderRadius: 99, backgroundColor: slice.color }}
                />
              </div>
            </div>
          ))}
...
function BudgetVsActual({
  ...
  const actualByCategory = monthExpenses.reduce<Record<string, number>>((acc, e) => {
    acc[e.category] = (acc[e.category] ?? 0) + Number(e.amount)
    return acc
  }, {})

  const plannedByCategory = lineItems.reduce<Record<string, number>>((acc, i) => {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${colors.border}` }}>
              {['Category', 'Planned', 'Actual', 'Variance', 'Usage'].map((h) => (
                <th
                  key={h}
                  className="text-left py-2 px-3"
                  style={{ color: colors.textSecondary, fontWeight: 600, fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.05em' }}
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.cat} style={{ borderBottom: `1px solid ${colors.border}` }}>
                <td className="py-3 px-3" style={{ color: colors.textPrimary }}>{row.cat}</td>
                <td className="py-3 px-3" style={{ color: colors.textSecondary }}>{fmt(row.planned)}</td>
                <td className="py-3 px-3" style={{ color: colors.textSecondary }}>{fmt(row.actual)}</td>
                <td className="py-3 px-3" style={{ color: row.over ? colors.errorText : colors.successText, fontWeight: 600 }}>
          ...
          <tfoot>
            <tr style={{ backgroundColor: colors.warmLinen }}>
              ...
              <td className="py-3 px-3 font-bold" style={{ color: colors.textPrimary }}>{fmt(totalPlanned)}</td>
              <td className="py-3 px-3 font-bold" style={{ color: colors.textPrimary }}>{fmt(totalActual)}</td>
              <td className="py-3 px-3 font-bold" style={{ color: totalVariance < 0 ? colors.errorText : colors.successText }}>
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: '100%',
                        borderRadius: '99px',
                        backgroundColor: totalActual > totalPlanned ? colors.errorText : colors.mistyForest,
                      }}
                    />
                  </div>
                  <span style={{ color: colors.textSecondary, fontSize: '12px', minWidth: '38px', textAlign: 'right', fontWeight: 600 }}>
...
function BudgetTab({
  ...
  async function addItem() {
    ...
    setSaving(true)
    await db.schema('budget').from('line_items').insert({
    ...
    setSaving(false)
    setShowAdd(false)
    setNewItem({ category: BUDGET_CATEGORIES[0], item_name: '', planned_amount: '', notes: '' })
    onRefresh()
  }
    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewItem((p) => ({ ...p, planned_amount: e.target.value }))}
              />
            </div>
            <div>
      ...
      <Table headers={['Category', 'Item', 'Planned Amount', 'Notes', '']}>
            ...
            <React.Fragment key={`cat-${cat}`}>
              <tr style={{ backgroundColor: '#F6F1E8' }}>
                <td
                  ...
                  style={{ color: colors.mistyForest, borderBottom: `1px solid #E8E4DF` }}
                >
                  {cat} — {fmt(catTotal)}
                </td>
              </tr>
        ...
        <tr style={{ backgroundColor: '#F6F1E8', borderTop: `1px solid #E8E4DF` }}>
          <td colSpan={2} className="px-4 py-3 text-sm font-bold" style={{ color: colors.mistyForest }}>
            Total Monthly Budget
          </td>
          <td className="px-4 py-3 text-sm font-bold" style={{ color: colors.mistyForest }}>
            {fmt(total)}
          </td>
          <td colSpan={2} />
        </tr>
...
function ExpensesTab({
  ...
  async function addExpense() {
    ...
    setSaving(true)
    await db.schema('budget').from('expenses').insert({
  ...
  return (
    <div className="space-y-5">
          ...
          <>
            ...
            >
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
                <div className="space-y-4">
                  {[
                    { label: 'Name', key: 'expense_name', type: 'text', placeholder: 'e.g. Zoho Mail' },
                    { label: 'Amount ($)', key: 'amount', type: 'number', placeholder: '0.00' },
                    { label: 'Date', key: 'expense_date', type: 'date', placeholder: '' },
                    { label: 'Payment Method', key: 'payment_method', type: 'text', placeholder: 'Card / ACH / Check' },
                    { label: 'Notes', key: 'notes', type: 'text', placeholder: 'Optional' },
                  ].map(({ label, key, type, placeholder }) => (
                    <div key={key}>
                      <label className="text-xs mb-1 block" style={{ color: colors.textSecondary }}>{label}</label>
                      <input
                        style={inputStyle}
                        type={type}
              ...
              >
                <button style={btnPrimary} onClick={addExpense} disabled={saving}>
      ...
      <Table headers={['Expense Name', 'Category', 'Amount', 'Payment Method', 'Date', 'Notes', 'Actions']}>
        {filtered.length === 0 ? (
            ...
            <TableRow key={exp.id} index={i}>
              ...
              <TableCell>
                <div className="flex gap-2">
                  ...
                  <button style={btnDanger} onClick={() => deleteExpense(exp.id)}>Del</button>
                </div>
              </TableCell>
            </TableRow>
          ))
        ...
        <tr style={{ backgroundColor: '#F6F1E8', borderTop: `1px solid #E8E4DF` }}>
          <td colSpan={2} className="px-4 py-3 text-sm font-bold" style={{ color: colors.textPrimary }}>Total</td>
          <td className="px-4 py-3 text-sm font-bold" style={{ color: colors.errorText }}>{fmt(total)}</td>
          <td colSpan={4} />
        </tr>
      </Table>
    </div>
...
function RevenueTab({
  ...
  async function addIncome() {
    ...
    setSaving(true)
    await db.schema('budget').from('income').insert({
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        {showAdd && (
          ...
          >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-3">
              <div>
                <label className="text-xs mb-1 block" style={{ color: colors.textSecondary }}>Source</label>
                <select
                  style={inputStyle}
                  value={newIncome.source}
                  onChange={(e) =>
                    setNewIncome((p) => ({ ...p, source: e.target.value as BudgetIncome['source'] }))
                  }
                >
              ...
              <div>
                <label className="text-xs mb-1 block" style={{ color: colors.textSecondary }}>Student Name</label>
                <input
                  style={inputStyle}
                  placeholder="Optional"
                  value={newIncome.student_name}
                  onChange={(e) => setNewIncome((p) => ({ ...p, student_name: e.target.value }))}
                />
              </div>
              <div>
                <label className="text-xs mb-1 block" style={{ color: colors.textSecondary }}>Amount ($)</label>
                <input
                  style={inputStyle}
                  type="number"
                  placeholder="0.00"
              ...
              <div>
                <label className="text-xs mb-1 block" style={{ color: colors.textSecondary }}>Date</label>
                <input
                  style={inputStyle}
                  type="date"
                  value={newIncome.income_date}
                  onChange={(e) => setNewIncome((p) => ({ ...p, income_date: e.target.value }))}
                />
              </div>
              <div>
                <label className="text-xs mb-1 block" style={{ color: colors.textSecondary }}>Description</label>
                <input
                  style={inputStyle}
                  placeholder="Optional"
        ...
        <Table headers={['Date', 'Source', 'Student', 'Description', 'Amount', '']}>
          {income.length === 0 ? (
            <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-gray-400">No income entries yet.</td></tr>
          ) : (
            income.map((inc, i) => (
              <TableRow key={inc.id} index={i}>
                <TableCell>{inc.income_date}</TableCell>
                <TableCell>
                  <span style={{ backgroundColor: colors.pastelSage, color: colors.mistyForest, borderRadius: '99px', padding: '2px 8px', fontSize: '11px', fontWeight: 600 }}>
                    {SOURCE_LABELS[inc.source]}
                  </span>
                </TableCell>
                <TableCell>{inc.student_name ?? '—'}</TableCell>
                <TableCell>{inc.description ?? '—'}</TableCell>
                <TableCell className="font-semibold" style={{ color: colors.successText }}>{fmt(Number(inc.amount))}</TableCell>
                <TableCell>
                  <button style={btnDanger} onClick={() => deleteIncome(inc.id)}>Del</button>
                </TableCell>
              </TableRow>
            ))
          )}
          {/* Total row */}
          <tr style={{ backgroundColor: '#F6F1E8', borderTop: `1px solid #E8E4DF` }}>
            <td colSpan={4} className="px-4 py-3 text-sm font-bold" style={{ color: colors.textPrimary }}>Total</td>
            <td className="px-4 py-3 text-sm font-bold" style={{ color: colors.successText }}>{fmt(totalActual)}</td>
            <td />
          </tr>
        </Table>
      </div>
      ...
      <div style={{ ...cardStyle, padding: '24px' }}>
          ...
          {[
            ...
            <div key={key}>
              <label className="text-xs mb-1 block" style={{ color: colors.textSecondary }}>
                {label}{' '}
                <span style={{ color: colors.textTertiary }}>({fmt(rate)}/mo)</span>
              </label>
              <input
                ...
                onChange={(e) =>
                  setEnrollment((p) => ({ ...p, [key]: parseInt(e.target.value) || 0 }))
                }
              />
        ...
        >
          <div>
            ...
            <p className="text-2xl font-bold" style={{ color: colors.mistyForest }}>
          ...
          <div className="text-right">
            ...
            <p
              ...
              style={{ color: totalActual >= projectedRevenue ? colors.successText : colors.errorText }}
            >
    ...
    <div className="space-y-6">
      ...
      <div style={{ ...cardStyle, padding: '24px' }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {[
            { label: 'Federal Corporate Rate (%)', value: federalRate, min: 0, max: 50, step: 1, setter: setFederalRate },
            { label: 'TX Franchise Tax Rate (%)', value: txRate, min: 0, max: 5, step: 0.125, setter: setTxRate },
            { label: 'Total Reserve % (of profit)', value: totalTaxRate, min: 0, max: 50, step: 1, setter: setTotalTaxRate },
          ].map(({ label, value, min, max, step, setter }) => (
            <div key={label}>
              <div className="flex justify-between mb-1">
                <label className="text-xs" style={{ color: colors.textSecondary }}>{label}</label>
                <span className="text-xs font-semibold" style={{ color: colors.mistyForest }}>{value}%</span>
              </div>
              <input
                type="range"
                min={min}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <MiniStat label="Current Month Profit" value={fmt(netProfit)} color={netProfit >= 0 ? colors.successText : colors.errorText} />
        <MiniStat label={`Tax Reserve (${totalTaxRate}% of profit)`} value={fmt(taxReserve)} color={colors.warningText} delay={0.05} />
        <MiniStat label="After-Tax Net" value={fmt(Math.max(netProfit - taxReserve, 0))} color={colors.mistyForest} delay={0.1} />
      </div>
        ...
        <div style={{ ...cardStyle, padding: '24px' }}>
          ...
          <div className="space-y-3">
            {quarters.map(({ q, amount }) => (
              ...
              >
                <span className="text-sm font-medium" style={{ color: colors.textPrimary }}>{q}</span>
                <span className="text-sm font-bold" style={{ color: colors.warningText }}>{fmt(amount)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ ...cardStyle, padding: '24px' }}>
          ...
          <div className="space-y-3">
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs" style={{ color: colors.textTertiary }}>{note}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs" style={{ color: colors.textSecondary }}>Gross: {fmt(gross)}</p>
                    <p className="text-sm font-bold" style={{ color: colors.errorText }}>Tax: {fmt(tax)}</p>
                  </div>
                </div>
              </div>
            ))}
            <div
              ...
              style={{ backgroundColor: colors.pastelSage }}
            >
              <span className="text-sm font-bold" style={{ color: colors.mistyForest }}>
...
function MixSummary({
  ...
  const barColor = surplus ? '#5E7C68' : mixTotal / goal >= 0.5 ? '#C07A4A' : '#C0524A'
  const profitColor =
    mixProfit >= targetProfit ? colors.successText
    : mixProfit >= targetProfit * 0.85 ? '#C07A4A'
    : colors.errorText
  return (
    <div
      style={{
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-4 mb-3">
        <div>
          <p className="text-xs mb-0.5" style={{ color: colors.textTertiary }}>Revenue</p>
          <p className="text-sm font-bold" style={{ color: colors.textPrimary }}>{fmt(mixTotal)}/mo</p>
        </div>
        <div>
          <p className="text-xs mb-0.5" style={{ color: colors.textTertiary }}>Projected profit</p>
          <p className="text-sm font-bold" style={{ color: profitColor }}>{fmt(mixProfit)}/mo</p>
        </div>
        <div>
          <p className="text-xs mb-0.5" style={{ color: colors.textTertiary }}>Your goal</p>
          <p className="text-sm font-bold" style={{ color: colors.textPrimary }}>{fmt(targetProfit)}/mo</p>
        </div>
      </div>
      <div style={{ height: 5, borderRadius: 99, backgroundColor: colors.warmLinen, overflow: 'hidden' }}>
        <motion.div
      ...
      <p className="text-xs mt-2 font-medium" style={{ color: surplus ? colors.successText : colors.errorText }}>
...
function RevenueGoalHeader({
  revenueGoal, mixTotal, budget, targetProfit,
}: {
  revenueGoal: number; mixTotal: number; budget: number; targetProfit: number
}) {
  const diff = mixTotal - revenueGoal
  const surplus = diff >= 0
  const barPct = Math.min(mixTotal / revenueGoal, 1)
  const barColor = barPct >= 1 ? '#5E7C68' : barPct >= 0.75 ? '#C07A4A' : '#C0524A'
  return (
    <div>
      <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide mb-0.5" style={{ color: colors.textSecondary }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <p className="text-xs mb-0.5" style={{ color: colors.textTertiary }}>Revenue needed</p>
          <p className="text-xl font-bold" style={{ color: colors.mistyForest }}>{fmt(revenueGoal)}/mo</p>
          <p className="text-xs mt-0.5" style={{ color: colors.textTertiary }}>
            {fmt(budget)} budget + {fmt(targetProfit)} profit
          </p>
        </div>
      </div>
      <div style={{ position: 'relative', height: 8, borderRadius: 99, backgroundColor: colors.warmLinen }}>
        <motion.div
          animate={{ width: `${barPct * 100}%` }}
          transition={{ duration: 0.3, ease: 'easeOut' }}
          style={{ height: '100%', borderRadius: 99, backgroundColor: barColor, maxWidth: '100%' }}
        />
        {/* Goal line at 100% */}
        <div style={{ position: 'absolute', right: 0, top: -4, bottom: -4, width: 2, borderRadius: 1, backgroundColor: colors.mistyForest }} />
      </div>
      <div className="flex justify-between items-baseline mt-1.5">
        <span className="text-xs" style={{ color: colors.textTertiary }}>
          Current mix: <strong style={{ color: colors.textPrimary }}>{fmt(mixTotal)}/mo</strong>
        </span>
        <span className="text-xs font-semibold" style={{ color: surplus ? colors.successText : colors.errorText }}>
          {surplus ? `+${fmt(diff)} above goal` : `${fmt(Math.abs(diff))} to go`}
        </span>
      </div>
    </div>
  )
...
function SupplementalStepper({
  label, rateLabel, rate, count, onChange, maxCount = 20,
}: {
  label: string; rateLabel: string; rate: number
  count: number; onChange: (n: number) => void; maxCount?: number
}) {
  ...
  const stepBtn: React.CSSProperties = {
    ...
    color: colors.textPrimary, display: 'flex',
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <span className="ml-2 text-xs" style={{ color: colors.textTertiary }}>{rateLabel}</span>
      </div>
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          <button style={stepBtn} onClick={() => onChange(Math.max(0, count - 1))}>−</button>
          <span style={{ minWidth: 28, textAlign: 'center', fontSize: 15, fontWeight: 700, color: colors.textPrimary }}>
            {count}
          </span>
          <button style={stepBtn} onClick={() => onChange(Math.min(maxCount, count + 1))}>+</button>
        </div>
        <span className="text-sm font-semibold whitespace-nowrap" style={{
          color: count > 0 ? colors.mistyForest : colors.textTertiary, minWidth: 100,
        }}>
...
function CoreSliderRow({
  ...
  const barColor = barPct >= 1 ? '#5E7C68' : barPct >= 0.5 ? '#C07A4A' : '#C0524A'
  const covered = targetCount >= neededAlone
  return (
    <div>
      <div className="flex flex-wrap items-baseline justify-between gap-1 mb-1.5">
        <span className="font-medium" style={{ fontSize: 14, color: colors.textPrimary }}>
          {label}
          <span className="ml-2 font-normal" style={{ fontSize: 11, color: colors.textTertiary }}>{rateLabel}</span>
        </span>
        <span className="text-xs" style={{ color: colors.textTertiary }}>
      ...
      <div className="flex items-center gap-3 mb-1.5">
        ...
        />
        <div style={{ textAlign: 'right' }}>
          <div className="text-xs font-semibold whitespace-nowrap" style={{ color: colors.textPrimary }}>
            {targetCount} students
          </div>
          <div className="text-xs font-medium whitespace-nowrap" style={{ color: colors.textSecondary }}>
            {contributionLabel}
          </div>
        </div>
      </div>
      <div style={{ height: 6, borderRadius: 99, backgroundColor: colors.warmLinen, overflow: 'hidden' }}>
        <motion.div
          animate={{ width: `${barPct * 100}%` }}
          transition={{ duration: 0.3, ease: 'easeOut' }}
          style={{ height: '100%', borderRadius: 99, backgroundColor: barColor }}
...
const SCHOOL_YEAR_RATES = [
  { key: 'full_14', label: 'Full Enrollment (1st–4th)', rate: TUITION_RATES.full_14 },
  { key: 'full_primary', label: 'Full Enrollment (Primary, Pre-K–K)', rate: TUITION_RATES.full_primary },
  { key: 'aftercare_enrolled', label: 'After Care – enrolled student', rate: TUITION_RATES.aftercare_enrolled },
  { key: 'aftercare_non', label: 'After Care – after care only', rate: TUITION_RATES.aftercare_non },
  { key: 'fun_friday', label: 'Field Day Friday (pkg of 4)', rate: TUITION_RATES.fun_friday },
]

const SUMMER_WEEKLY_RATES = [
  { key: 'summer_14_wk', label: 'Weekly (1st–4th)', rate: TUITION_RATES.summer_14_wk },
  { key: 'summer_primary_wk', label: 'Weekly Primary', rate: TUITION_RATES.summer_primary_wk },
]

const SUMMER_SEASON_RATES = [
  { key: 'full_season_14', label: 'Full Season (1st–4th)', totalPrice: FULL_SUMMER_14 },
  { key: 'full_season_primary', label: 'Full Season Primary', totalPrice: FULL_SUMMER_PRIMARY },
]
...
function AnalysisTab({
  ...
  const [ssStudents, setSsStudents] = useState<Record<string, number>>(() =>
    Object.fromEntries(SUMMER_SEASON_RATES.map(r => [r.key, Math.ceil(((totalBudget + 500) * 3) / r.totalPrice)]))
  )
    ...
    <div className="space-y-6">
      ...
      <div style={{ ...cardStyle, padding: '32px' }}>
        <div className="text-center mb-6">
          ...
          <p className="text-5xl font-bold mb-1" style={{ color: colors.mistyForest }}>
            {fmt(targetProfit)}
          </p>
          <p className="text-sm" style={{ color: colors.textSecondary }}>per month profit</p>
        </div>
        <input
          type="range"
          min={0}
          ...
          style={{ width: '100%', accentColor: colors.mistyForest }}
        />
        <div className="flex justify-between text-xs mt-1 mb-4" style={{ color: colors.textTertiary }}>
          <span>$0</span>
          <span>$5,000</span>
        </div>
        <div className="flex justify-center">
          <div className="text-sm px-4 py-2 rounded-full" style={{ backgroundColor: colors.softCloud }}>
            <span style={{ color: colors.textSecondary }}>Current actual profit: </span>
            <span className="font-bold" style={{ color: netProfitColor }}>{fmt(netProfit)}</span>
          </div>
        </div>
      </div>
        ...
        {(['School Year', 'Summer'] as const).map(tab => (
          <button
            ...
            style={{
              ...
              color: activeSubTab === tab ? colors.textPrimary : colors.textSecondary,
      ...
      {activeSubTab === 'School Year' && (
        <div style={{ ...cardStyle, padding: '24px' }}>
          {(() => {
            const syMixTotal = SCHOOL_YEAR_RATES.reduce((s, { key, rate }) => s + (syStudents[key] ?? 0) * rate, 0)
            return (
              <>
                <RevenueGoalHeader revenueGoal={totalBudget + targetProfit} mixTotal={syMixTotal} budget={totalBudget} targetProfit={targetProfit} />
                ...
                <div className="space-y-6 mb-6">
                  {CORE_SY_RATES.map(({ key, label, rate }) => {
                    const neededAlone = Math.ceil((totalBudget + targetProfit) / rate)
                    ...
                    return (
                      <CoreSliderRow
                        ...
                        onChange={n => setSyStudents(p => ({ ...p, [key]: n }))}
                      />
                    )
                  })}
                </div>
                ...
                <div style={{ borderTop: `1px solid ${colors.border}`, paddingTop: 16 }}>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span style={{
                      ...
                      borderRadius: 99, backgroundColor: colors.info, color: colors.infoText,
                    }}>
                      bonus revenue
                    </span>
                  </div>
                  <p className="text-xs mb-4" style={{ color: colors.textTertiary }}>
                    These supplement core revenue — no target needed, every student is a bonus.
                  </p>
                  <div className="space-y-4">
                ...
                <MixSummary
                  ...
                  onReset={() => setSyStudents({
                    full_14: 0, full_primary: 0, aftercare_enrolled: 0, aftercare_non: 0, fun_friday: 0,
                  })}
                />
              </>
            )
      ...
      {activeSubTab === 'Summer' && (
        <div style={{ ...cardStyle, padding: '24px' }}>
          <div className="flex flex-wrap items-baseline justify-between gap-2 mb-1">
            <p className="text-sm font-semibold uppercase tracking-wide" style={{ color: colors.textPrimary }}>
              Summer Program · 13 weeks · Mon–Thu
            </p>
          </div>
          <p className="text-xs mb-5" style={{ color: colors.textTertiary }}>
            Weekly rates shown as monthly equivalent (×4.33). Full season is total price per student for the 13-week summer.
          </p>
          ...
          <div className="space-y-6 mb-4">
            {SUMMER_WEEKLY_RATES.map(({ key, label, rate }) => {
              const neededAlone = Math.ceil((totalBudget + targetProfit) / (rate * 4.33))
          ...
          <MixSummary
            mixTotal={SUMMER_WEEKLY_RATES.reduce((s, { key, rate }) => s + (swStudents[key] ?? 0) * rate * 4.33, 0)}
            goal={totalBudget + targetProfit}
            label="Weekly"
            budget={totalBudget}
            targetProfit={targetProfit}
            onReset={() => setSwStudents(Object.fromEntries(SUMMER_WEEKLY_RATES.map(r => [r.key, 0])))}
          />
          ...
          <div className="space-y-6">
            {SUMMER_SEASON_RATES.map(({ key, label, totalPrice }) => {
              const neededAlone = Math.ceil(((totalBudget + targetProfit) * 3) / totalPrice)
          ...
          <MixSummary
            mixTotal={SUMMER_SEASON_RATES.reduce((s, { key, totalPrice }) => s + (ssStudents[key] ?? 0) * totalPrice, 0) / 3}
            goal={totalBudget + targetProfit}
            label="Full season (÷3 months)"
            budget={totalBudget}
            targetProfit={targetProfit}
            onReset={() => setSsStudents(Object.fromEntries(SUMMER_SEASON_RATES.map(r => [r.key, 0])))}
          />
        </div>
      )}
    </div>
...
export default function BudgetPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [liRes, expRes, incRes] = await Promise.all([
      ...
      db.schema('budget').from('expenses').select('*').order('expense_date', { ascending: false }),
      db.schema('budget').from('income').select('*').order('income_date', { ascending: false }),
    ])
    if (liRes.error || expRes.error || incRes.error) {
      setError('Failed to load budget data. Make sure you have super_admin access.')
    } else {
  ...
  return (
    <div className="flex-1 overflow-auto" style={{ backgroundColor: colors.softCloud }}>
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div
          ...
          style={{ backgroundColor: colors.warmLinen, border: `1px solid ${colors.border}` }}
        >
          {TABS.map((tab) => (
            <button
              key={tab}
              ...
              style={{
                ...
                color: activeTab === tab ? colors.mistyForest : colors.textSecondary,
        ...
        {loading ? (
          ...
          >
            <div className="text-center">
              <div
                ...
                style={{ borderColor: colors.mistyForest, borderTopColor: 'transparent' }}
              />
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === 'Overview' && (
                <OverviewTab lineItems={lineItems} expenses={expenses} income={income} />
              )}
              {activeTab === 'Budget' && (
                <BudgetTab lineItems={lineItems} expenses={expenses} onRefresh={fetchAll} />
              )}
              {activeTab === 'Expenses' && (
              ...
              {activeTab === 'Analysis' && (
                <AnalysisTab lineItems={lineItems} expenses={expenses} income={income} />
              )}
            </motion.div>
          </AnimatePresence>
        )}
