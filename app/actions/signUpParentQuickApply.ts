...
export async function signUpParentQuickApply(fullName: string, email: string, password: string) {
  ...
  if (error) {
    void sendDiscordNotification(createErrorEmbed({ context: 'signUpParentQuickApply – Auth SignUp', error: error.message, details: { email } })).catch(() => {})
  ...
  if (signInError) {
    void sendDiscordNotification(createErrorEmbed({ context: 'signUpParentQuickApply – Auth SignIn', error: signInError.message, details: { email } })).catch(() => {})
  ...
  if (insertError) {
    void sendDiscordNotification(createErrorEmbed({ context: 'signUpParentQuickApply – DB Insert user', error: insertError.message, details: { email, userId } })).catch(() => {})
  ...
  return { redirectTo: '/quick-apply/step/1?new=1' }
}
